...
int main(int argc, const char* argv[])
{
  ...
  /*
  darc::NodeLinkUDP::Ptr link1( new darc::NodeLinkUDP(&(node->io_service_), 7002) );
  link1->addRemoteNode(1, "127.0.0.1", "7001");
  node->remote_dispatch_handler_.addRemoteLink(1, link1);
  */

  node->remote_dispatch_handler_.setNodeID(125);
  node->remote_dispatch_handler_.accept("udp://127.0.0.1:5006");
